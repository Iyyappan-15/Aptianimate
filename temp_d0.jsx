// src/pages/TopicPage.jsx ΓÇö Premium Topic Learning Experience
...
const PRACTICE_LEVELS = [
  { id: "easy",   emoji: "≡ƒƒó", label: "Easy Practice",    count: 25, color: "#10b981", desc: "Build confidence with straightforward questions" },
  { id: "medium", emoji: "≡ƒƒí", label: "Medium Practice",  count: 25, color: "#f59e0b", desc: "Standard exam-level questions with twists" },
  { id: "hard",   emoji: "≡ƒƒá", label: "Hard Practice",    count: 25, color: "#f97316", desc: "Tricky questions that trip most candidates" },
  { id: "mixed",  emoji: "≡ƒö┤", label: "Mixed Challenge",  count: 100, color: "#ef4444", desc: "All levels combined for real exam simulation" },
];

// ΓöÇΓöÇΓöÇ Universal answer resolver ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Handles 3 formats found in different JSON files:
//   1. number  (0-3) ΓÇö most generated topics (e.g. percentages, error-spotting)
//   2. letter  (A/B/C/D) ΓÇö number-series, letter-series etc.
//   3. string  (exact text) ΓÇö number-system-basics
...
function LessonSlide({ lesson, color, topicName, onNext, isLast }) {
  const nextBtn = (
    <button onClick={onNext} style={{ marginTop: 28, padding: "14px 32px", borderRadius: 12, border: "none", background: "linear-gradient(135deg," + color + "," + color + "bb)", color: "#fff", fontWeight: 800, fontSize: "1rem", cursor: "pointer", transition: "transform .15s" }}>
      {isLast ? "Go to Practice Mode ≡ƒÄ»" : "Got it! Next ΓåÆ"}
    </button>
  );
...
export default function TopicPage({ topicSlug, topicName, navigate }) {
  ...
  // ΓöÇΓöÇΓöÇ Load questions when practice level selected ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    ...
    fetch(url)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status} ΓÇö ${url}`);
        return res.json();
      })
      .then(data => {
  ...
  if (!topic) return (
    <div className="topic-page page">
      <button className="topic-back-btn" onClick={() => navigate("")}>ΓåÉ Back</button>
      <div className="topic-not-found">
        <div style={{ fontSize: "3rem" }}>≡ƒÜº</div>
  ...
  if (screen === "overview") return (
    <div className="topic-page page" style={{ animation:"fadeIn .4s ease",maxWidth:860,margin:"0 auto",padding:"24px 16px 80px" }}>
      <button className="topic-back-btn" onClick={() => history.back()}>ΓåÉ Back</button>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:20,marginBottom:32 }}>
        ...
        <div onClick={startLearn} style={{ background:"linear-gradient(135deg,"+color+"18,"+color+"06)",border:"2px solid "+color,borderRadius:20,padding:28,cursor:"pointer",transition:"transform .2s,box-shadow .2s",position:"relative",overflow:"hidden" }}
          ...
          <div style={{ position:"absolute",top:14,right:14,background:color,color:"#fff",fontSize:".68rem",fontWeight:800,padding:"4px 10px",borderRadius:100,letterSpacing:"1px" }}>Γ¡É RECOMMENDED</div>
          <div style={{ fontSize:"2.5rem",marginBottom:12 }}>≡ƒôÜ</div>
          ...
          {["Step-by-step visual lessons","Interactive formula animations","Memory tricks & shortcuts","Guided AI practice"].map(f=><div key={f} style={{ display:"flex",alignItems:"center",gap:8,fontSize:".85rem",color:"var(--text-sec)",marginBottom:4 }}><span style={{ color:"#10b981" }}>Γ£ô</span>{f}</div>)}
          <div style={{ marginTop:16,fontSize:".8rem",color:"var(--text-sec)",marginBottom:16 }}>ΓÅ▒ {meta.time}ΓÇô{meta.time+10} mins</div>
          <button style={{ width:"100%",padding:14,borderRadius:12,border:"none",background:"linear-gradient(135deg,"+color+","+color+"bb)",color:"#fff",fontWeight:800,fontSize:"1rem",cursor:"pointer" }}>Start Learning ΓåÆ</button>
        </div>
        ...
        <div onClick={startPractice} style={{ background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20,padding:28,cursor:"pointer",transition:"transform .2s,box-shadow .2s" }}
          ...
          onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none";}}>
          <div style={{ fontSize:"2.5rem",marginBottom:12 }}>≡ƒÄ»</div>
          ...
          {["Choose difficulty level","AI animated step-by-step solutions","Accuracy & speed tracking","Shortcut methods & mistake analysis"].map(f=><div key={f} style={{ display:"flex",alignItems:"center",gap:8,fontSize:".85rem",color:"var(--text-sec)",marginBottom:4 }}><span style={{ color:"#f59e0b" }}>Γ£ô</span>{f}</div>)}
          <button style={{ width:"100%",padding:14,borderRadius:12,border:"1px solid var(--border)",background:"var(--surface2)",color:"var(--text-main)",fontWeight:800,fontSize:"1rem",cursor:"pointer",marginTop:32 }}>Start Practice ΓåÆ</button>
        </div>
      </div>
      ...
      {topic.keyFacts && topic.keyFacts.length > 0 && (
        <div style={{ background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20,padding:28 }}>
          <h3 style={{ fontSize:"1.1rem",fontWeight:800,color:"var(--text-main)",marginBottom:16 }}>≡ƒôî Topics Covered</h3>
    ...
    return (
      <div className="topic-page page" style={{ animation:"fadeIn .4s ease",maxWidth:760,margin:"0 auto",padding:"24px 16px 80px" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12 }}>
          <button className="topic-back-btn" style={{ margin:0 }} onClick={() => setScreen("overview")}>ΓåÉ Overview</button>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:20 }}>
          ...
          <span style={{ fontSize:".9rem",fontWeight:700,color:"var(--text-sec)" }}>≡ƒôÜ Learn Mode ΓÇö</span>
  ...
  /* ΓöÇΓöÇ PRACTICE MODE ΓöÇΓöÇ */
  if (screen === "practice") {

    // STEP 1: Show level selection if no level chosen
    if (!practiceLevel) return (
      <div className="topic-page page" style={{ animation:"fadeIn .4s ease",maxWidth:760,margin:"0 auto",padding:"24px 16px 80px" }}>
        <button className="topic-back-btn" onClick={() => setScreen("overview")}>ΓåÉ Overview</button>
        <div style={{ textAlign:"center",marginBottom:32 }}>
          <div style={{ fontSize:"3rem",marginBottom:12 }}>≡ƒÄ»</div>
    ...
    if (loadingQuestions) return (
      <div className="topic-page page" style={{ animation:"fadeIn .4s ease",maxWidth:760,margin:"0 auto",padding:"24px 16px 80px" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24 }}>
          <button className="topic-back-btn" style={{ margin:0 }} onClick={() => setPracticeLevel(null)}>ΓåÉ Change Level</button>
          <div style={{ fontSize:".9rem",color:"var(--text-sec)",fontWeight:600 }}>≡ƒÄ» {practiceLevel.emoji} {practiceLevel.label} ┬╖ {practiceLevel.count} Questions</div>
        </div>
        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:300,gap:20 }}>
          ...
          <p style={{ color:"var(--text-sec)",fontWeight:600,fontSize:"1rem" }}>Loading {practiceLevel.label} questionsΓÇª</p>
        </div>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    );

    // STEP 3: Error state ΓÇö questions couldn't load
    if (errorMessage) return (
      <div className="topic-page page" style={{ animation:"fadeIn .4s ease",maxWidth:760,margin:"0 auto",padding:"24px 16px 80px" }}>
        ...
        <div style={{ textAlign:"center",padding:"60px 24px",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20 }}>
          <div style={{ fontSize:"3rem",marginBottom:16 }}>ΓÜá∩╕Å</div>
          <h3 style={{ color:"var(--text-main)",marginBottom:12 }}>Questions Unavailable</h3>
          <p style={{ color:"var(--text-sec)",fontSize:".9rem",marginBottom:24 }}>The question bank for <strong>{topic.title}</strong> ΓÇö {practiceLevel.label} could not be loaded. Please try a different level or come back later.</p>
          ...
          <div style={{ marginTop:24,display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap" }}>
            <button onClick={() => setPracticeLevel(null)} style={{ padding:"12px 28px",borderRadius:12,border:"1px solid var(--border)",background:"var(--surface2)",color:"var(--text-main)",fontWeight:700,cursor:"pointer" }}>ΓåÉ Try Another Level</button>
          </div>
        </div>
      </div>
    );
    ...
    if (!activeQuestion) return (
      <div className="topic-page page" style={{ maxWidth:760,margin:"0 auto",padding:"24px 16px 80px",textAlign:"center" }}>
        <button className="topic-back-btn" onClick={() => setPracticeLevel(null)}>ΓåÉ Change Level</button>
        <div style={{ padding:"60px 24px" }}>
          <div style={{ fontSize:"3rem",marginBottom:16 }}>≡ƒô¡</div>
          <p style={{ color:"var(--text-sec)" }}>No questions available for this level. Try another level.</p>
          <button onClick={() => setPracticeLevel(null)} style={{ marginTop:20,padding:"12px 28px",borderRadius:12,border:"none",background:color,color:"#fff",fontWeight:700,cursor:"pointer" }}>ΓåÉ Choose Another Level</button>
        </div>
      </div>
    );
    ...
    return (
      <div className="topic-page page" style={{ animation:"fadeIn .4s ease",maxWidth:760,margin:"0 auto",padding:"24px 16px 80px" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12 }}>
          <button className="topic-back-btn" style={{ margin:0 }} onClick={() => setPracticeLevel(null)}>ΓåÉ Change Level</button>
          <div style={{ fontSize:".9rem",color:"var(--text-sec)",fontWeight:600 }}>≡ƒÄ» {practiceLevel.emoji} {practiceLevel.label} ┬╖ {questionsList.length} Questions</div>
        </div>
        ...
        <div className="question-card" style={{ animation:"fadeIn 0.4s ease",padding:"32px",borderRadius:"20px",border:"1px solid var(--border)",background:"var(--surface)" }}>
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px" }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
              ...
              {/* Score */}
              <span style={{ fontSize:".8rem",fontWeight:700,color:"#10b981" }}>Γ£à {correctCount} correct</span>
              {/* Bookmark Button */}
              <button
              ...
              >
                {bookmarkLoading ? "ΓÅ│" : isBookmarked ? "≡ƒöû" : "≡ƒÅ│∩╕Å"}
              </button>
            </div>
          </div>
          ...
          <div style={{ fontSize:"0.85rem",color:"var(--text-sec)",marginBottom:"8px",fontWeight:500 }}>
            ≡ƒôî {activeQuestion.subtopic || "General"}
          </div>
              ...
              return (
                ...
                >
                  ...
                  {submitted && isThisCorrect && (
                    <span style={{ marginLeft:"auto",color:"var(--teal)",fontWeight:"bold",fontSize:"1.2rem" }}>Γ£ô</span>
                  )}
                  {submitted && isSelected && !isThisCorrect && (
                    <span style={{ marginLeft:"auto",color:"var(--coral)",fontWeight:"bold",fontSize:"1.2rem" }}>Γ£ù</span>
                  )}
                </button>
              );
            })}
          ...
          {submitted && (
            <div style={{ animation:"slideUp 0.3s ease",display:"flex",flexDirection:"column",gap:"20px" }}>
              ...
              <div style={{
                ...
                {isAnsweredCorrectly
                  ? "≡ƒÄë Correct! Great job."
                  : `Γ¥î Wrong. Correct answer: Option ${optionKeys[correctIdx]} ΓÇö ${getOptionString(correctText)}`
                }
              </div>
              ...
              {!isAnsweredCorrectly && activeQuestion.memoryTrick && (
                <div style={{ background:"var(--surface2)",padding:"16px 20px",borderRadius:"12px",fontSize:"0.9rem",color:"var(--text-sec)",borderLeft:"3px solid var(--amber)" }}>
                  ≡ƒÆí <strong>Memory Trick:</strong> {activeQuestion.memoryTrick}
                </div>
              )}
              ...
              {activeQuestion.explanation && (
                <div style={{ background:"var(--surface2)",padding:"16px 20px",borderRadius:"12px",fontSize:"0.9rem",color:"var(--text-sec)",borderLeft:"3px solid "+color }}>
                  ≡ƒôû <strong>Explanation:</strong> {activeQuestion.explanation}
                </div>
              )}
              ...
              <div style={{ display:"flex",gap:"16px",flexWrap:"wrap",marginTop:"8px" }}>
                ...
                <button
                  ...
                  onMouseOut={e => { e.currentTarget.style.transform="translateY(0)"; }}
                >
                  {isLastQuestion ? "≡ƒÅü Finish Practice" : "Γ₧í∩╕Å Next Question"}
                </button>
  ...
  if (screen === "complete") return (
    <div className="topic-page page" style={{ animation:"fadeIn .5s ease",maxWidth:600,margin:"0 auto",padding:"40px 16px 80px",textAlign:"center" }}>
      <div style={{ fontSize:"5rem",marginBottom:16 }}>≡ƒÅå</div>
      <h1 style={{ fontSize:"2rem",fontWeight:900,color:"#10b981",marginBottom:8 }}>Topic Mastered!</h1>
      <p style={{ color:"var(--text-sec)",marginBottom:32,fontSize:"1.05rem" }}>{topic.title} ΓÇö Completed</p>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:32 }}>
        {[{label:"Accuracy",value:stats?.accuracy+"%",color:"#10b981",icon:"≡ƒÄ»"},{label:"Time Taken",value:stats?.time+" min",color:"#6366f1",icon:"ΓÅ▒"},{label:"XP Earned",value:"+"+stats?.xp,color:"#f59e0b",icon:"ΓÜí"}].map(s=>(
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
        <button onClick={() => navigate("")} style={{ padding:"16px 32px",borderRadius:14,border:"none",background:"linear-gradient(135deg,"+color+","+color+"bb)",color:"#fff",fontWeight:800,fontSize:"1.05rem",cursor:"pointer" }}>Continue Journey ΓåÆ</button>
