  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--muted)', margin: '0 0 48px', fontSize: '1.1rem' }}>
        You scored {percent}% on today's randomly mixed set. Come back tomorrow!
      </p>
      ...
      <div style={{ display: 'flex', gap: '16px', justifyContent: 'center' }}>
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => navigate('govt-roadmaps')}
          style={{
...
export default function GovtDailyPracticePage({ navigate }) {
  ...
  useEffect(() => {
    ...
    const files = GOVT_PYQ_REGISTRY.flatMap(exam => exam.practiceSets.map(set => set.file));
    
    // 2. Fetch all questions
    Promise.all(files.map(file => fetch(file).then(res => res.ok ? res.json() : [])))
      .then(results => {
        let allQuestions = results.flat();
        
        // 3. Shuffle and pick exactly 20 questions
  ...
  if (error) {
    return (
      <div className="page" style={{ textAlign: 'center', padding: '100px 24px' }}>
        ...
        <button onClick={() => navigate('govt-roadmaps')} className="btn btn-primary">Go Back</button>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="page" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '70vh' }}>
        ...
        <p style={{ color: 'var(--text-main)', marginTop: '24px', fontWeight: 800, fontSize: '1.2rem' }}>Generating Daily Mix…</p>
        <p style={{ color: 'var(--muted)', marginTop: '8px', fontSize: '0.9rem' }}>Pulling 20 fresh questions from the PYQ database</p>
      </div>
    );
  }
  ...
  if (showSummary) {
    return (
      <div className="page-wide" style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <ScoreSummary questions={questions} answers={answers} navigate={navigate} />
      </div>
    );
  }
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', background: 'var(--surface2)' }}>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-10%', left: '-5%', width: '50vw', height: '50vw', borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.05) 0%, transparent 60%)', filter: 'blur(60px)' }} />
        <div style={{ position: 'absolute', bottom: '-10%', right: '-5%', width: '50vw', height: '50vw', borderRadius: '50%', background: 'radial-gradient(circle, rgba(245,158,11,0.05) 0%, transparent 60%)', filter: 'blur(60px)' }} />
      </div>
      ...
      <div className="page-wide" style={{ position: 'relative', zIndex: 1, padding: '24px 24px 80px' }}>
        ...
        <div style={{
          ...
          marginBottom: '32px', boxShadow: 'var(--shadow)'
        }}>
          <button
            onClick={() => navigate('govt-roadmaps')}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--text-main)', letterSpacing: '-0.3px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span style={{ fontSize: '1.2rem' }}>⚡</span> Daily Practice
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '6px', background: 'linear-gradient(90deg, #10b981, #f59e0b)' }} />
                ...
                <div style={{ marginTop: '32px' }}>
                  {!submitted ? (
                    <motion.button
                      whileHover={selected ? { y: -2, boxShadow: '0 8px 24px rgba(16,185,129,0.25)' } : {}}
                      whileTap={selected ? { scale: 0.98 } : {}}
                      onClick={() => selected && handleSubmit(selected)}
                      disabled={!selected}
                      style={{
                        width: '100%', padding: '18px', borderRadius: '16px', border: 'none',
                        background: selected ? 'linear-gradient(135deg, #10b981, #059669)' : 'var(--surface2)',
            ...
            <div style={{
              ...
              <motion.button
                whileHover={{ x: 4, boxShadow: '0 8px 20px rgba(16,185,129,0.25)' }}
                whileTap={{ scale: 0.98 }}
                onClick={goNext}
                style={{
                  display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 24px',
                  borderRadius: '12px', border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)',
