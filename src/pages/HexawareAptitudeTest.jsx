    ...
    >
      <div style={{
        ...
        <p style={{ color: 'var(--muted)', fontSize: '1.1rem', marginBottom: '32px' }}>
          Complete simulation of the Hexaware assessment.
        </p>
...
// ──────────────────────────────────────────────────────────
export default function HexawareAptitudeTest({ navigate }) {
  ...
  const handleStart = useCallback(async () => {
    setPhase('loading');
    try {
      const config = mockTestService.getConfig('Hexaware');
      
      // Load both sections
      const [aptitudeQs, technicalQs] = await Promise.all([
        mockTestService.getAptitudeQuestions('Hexaware'),
        mockTestService.getTechnicalQuestions('Hexaware', 'Technical')
      ]);
  ...
  const handleOverallSubmit = useCallback(({ state }) => {
    ...
    navigate('Hexaware-ninja-mock/results');
  }, [navigate]);
